...
do
    ...
    do
        find . -name "*.${filetype}" -exec sed -i "s/${ARC_COLORS[$index]}/${MONOCHROME_COLORS[$index]}/Ig" {} \;
    done
done
echo "## Replacing in gtk-2.0 rc"
for index in ${!ARC_COLORS[*]}
do
    find . -type f -name "gtkrc*" -exec sed -i "s/${ARC_COLORS[$index]}/${MONOCHROME_COLORS[$index]}/Ig" {} \;
done
